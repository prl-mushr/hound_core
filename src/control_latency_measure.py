...
def output_cb(msg):
    ...
    latency = min(
        max(0, output_time - high_level_time),
        max(0, 0.02 - (output_time - high_level_time)),
    )
...
hlc_sub = rospy.Subscriber(
    "/low_level_controller/hound/control", AckermannDriveStamped, HLC_cb
)
