...
def get_state_data(imu_msg, odom_msg):
    ...
    state[6:9] = [odom_msg.twist.twist.linear.x, odom_msg.twist.twist.linear.y, odom_msg.twist.twist.linear.z]
    state[12:15] = [odom_msg.twist.twist.angular.x, odom_msg.twist.twist.angular.y, odom_msg.twist.twist.angular.z]
    state[9:12] = [imu_msg.linear_acceleration.x, imu_msg.linear_acceleration.y, imu_msg.linear_acceleration.z]
    return state
...
def grid_map_callback(grid_map):
    ...
    valid = np.array([1,1,0,0,0]) ## valid variable tells us which of these maps are "valid"
    map_color = cv2.applyColorMap(((map_elev + 4)*255/8).astype(np.uint8), cv2.COLORMAP_JET)
...
def main(Config):
    ...
    topic_list = ['/sensors/core', '/mavros/imu/data_raw', '/mavros/local_position/odom', '/mavros/manual_control/send', '/mavros/rc/in', '/camera/color/image_raw', '/camera/depth/image_rect_raw', '/grid_map_occlusion_inpainting/all_grid_map']
    ...
    for bagdir_name in bagdir_list:
        ...
        for i in range(len(files)):
            ...
            print("tolerance state: {}, tolerance_camera: {}, sequence_length: {}".format(tolerance_state, tolerance_camera, sequence_length))
            ...
            for topic, msg, t in bag.read_messages(topics=topic_list):
                ...
                print("Data collection iter: {}".format(100*counter/ sequence_length), end='\r')
            state_data = []
            color = []
            depth = []
            map_elev = []
            ...
            for t1, color_msg in color_data.items():
                for t2, depth_msg in depth_data.items():
                    ...
                    if time_diff <= tolerance_camera:
                        ...
                        d = cv_bridge.imgmsg_to_cv2(depth_msg, desired_encoding="passthrough")
                        color.append(c)
                        depth.append(d)
            ...
            timestamps_camera = timestamps_camera[index_list] ## only those timestamps, color images and depth images for which we have corresponding elevation map
            ...
            print("pruned camera and state sequence_lengths:", len(timestamps) - state_index, len(timestamps_camera))

            while state_index < len(timestamps) and camera_index < len(timestamps_camera):
                state_timestamp = timestamps[state_index]
            ...
            timestamps = update_npy_datafile(timestamp_data, output_path / "timestamps.npy")
            ...
            camera_color_data = update_npy_datafile(camera_color_data, output_path / "camera_color.npy")
            camera_depth_data = update_npy_datafile(camera_depth_data, output_path / "camera_depth.npy")
...
if __name__ == "__main__":
    ...
    config_path = "/root/catkin_ws/src/BeamNGRL/Experiments/Configs/{}".format(config_name)
