...
class hal:
    def __init__(self, config_file):
        ...
        sub_mavros = rospy.Subscriber(
            self.mavros_config["monitor_topic"],
            rospy.AnyMsg,
            self.mavros_hz.callback_hz,
        )
        sub_camera = rospy.Subscriber(
            self.camera_config["monitor_topic"],
            rospy.AnyMsg,
            self.camera_hz.callback_hz,
        )

        sub_channel = rospy.Subscriber(
            self.mavros_config["channel_topic"], RCIn, self.channel_cb, queue_size=2
        )
        sub_voltage = rospy.Subscriber(
            self.vesc_config["topic"], VescStateStamped, self.voltage_cb, queue_size=1
        )
        sub_gps = rospy.Subscriber(
            self.mavros_config["gps_topic"], GPSRAW, self.GPS_cb, queue_size=1
        )
        pose_sub = rospy.Subscriber(
            self.mavros_config["pose_topic"], PoseStamped, self.pose_cb, queue_size=1
        )
        scan_sub = rospy.Subscriber(
            self.lidar_config["scan_topic"], LaserScan, self.scan_cb
        )
        mavros_status_sub = rospy.Subscriber(
            self.mavros_config["state_topic"],
            State,
            self.mavros_status_cb,
            queue_size=10,
        )
        ...
        self.pc_pub = rospy.Publisher(
            self.lidar_config["pc_topic"], PointCloud2, queue_size=1
        )
        self.last_pose_time = None

        self.notification_pub = rospy.Publisher(
            self.mavros_config["notification_topic"], PlayTuneV2, queue_size=10
        )
        ...
        self.diagnostics_pub = rospy.Publisher(
            config["diagnostics_topic"], DiagnosticArray, queue_size=2
        )

        time.sleep(15)
        os.system(
            self.mavros_config["failure_action"]
        )  ## first attempt at getting mavros to run @50 Hz
        self.main_loop()
    ...
    def start_recording(self):
        self.command = (
            "rosbag record --split --duration=5m -O "
            + self.bagdir
            + "temp "
            + self.record_command
        )
    ...
    def stop_recording(self):
        for proc in psutil.process_iter():
            if "record" in proc.name() and set(self.command[2:]).issubset(
                proc.cmdline()
            ):
    ...
    def GPS_cb(self, msg):
        if (
            msg.satellites_visible >= 16
            and msg.h_acc <= 1000
            and self.GPS_status == False
        ):
            self.publish_notification("GPS good")
            self.GPS_status = True
            time.sleep(1)
        elif (
            msg.satellites_visible < 16 or msg.h_acc > 1000
        ) and self.GPS_status == True:
    ...
    def pose_cb(self, msg):
        ...
        br.sendTransform((0, 0, 0), (0, 0, 0, 1), msg.header.stamp, "odom", "map")
    ...
    def mavros_status_cb(self, msg):
        ...
        if not armed and time.time() - self.last_map_clear > 1.0:
            ...
            )  ## worst (but easiest?) way to do a rosservice call when you don't care about returns
            self.last_map_clear = time.time()
    ...
    def publish_diagnostics(self):
        ...
        avg_gpu = 1e-9 * float(
            os.popen("cat /sys/devices/17000000.ga10b/devfreq/17000000.ga10b/max_freq")
            .read()
            .strip("\n")
        )
        ...
        diagnostics_status.values.append(
            KeyValue(key="mavros_init", value=str(self.mavros_init))
        )
        diagnostics_status.values.append(
            KeyValue(key="camera_init", value=str(self.camera_init))
        )
    ...
    def main_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                if (
                    mavros_freq >= 0.8 * self.mavros_config["fps"]
                    and self.mavros_init == False
                ):
                ...
                if (
                    camera_freq > 0.8 * self.camera_config["fps"]
                    and self.camera_init == False
                ):
